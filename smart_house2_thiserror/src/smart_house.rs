...
pub enum SmartHouseError {
    ...
    RoomIsNotFound,
    #[error("Room is already in house")]
    RoomIsAlreadyInHouse,
    #[error("There are no rooms")]
    EmptyRooms,
    #[error("Device is not found in room")]
    DeviceIsNotFoundInRoom,
    #[error("Device is already in room")]
    DeviceIsAlreadyInRoom,
}

#[derive(Error, Debug)]
pub enum SmartHouseReportErrors {
    #[error("Smart House components returned an error: {0}")]
    SmartHouseComponentsError(#[from] SmartHouseError),
    #[error("Device info providers returned an error: {0}")]
    ProvidersInfoError(#[from] DeviceInfoError),
}
#[derive(Error, Debug)]
...
//здесь не вышло заменить имплементациюю Display
